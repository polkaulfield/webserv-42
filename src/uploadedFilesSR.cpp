...
bool ServerResponse::_isAllowedType(std::string const &filename,
                                    std::string const &contentType) {
  const char *badTypes[] = {".exe", ".bat", ".cmd", ".com", ".pif",
                            ".scr", ".php", ".jsp", ".asp", NULL};
...
bool ServerResponse::_validateUploadedFile(UploadedFile const &file,
                                           const Config &config) {
  ...
  // Aqui se podria intentar sanitizar el nombre del archivo pero parece pateo y
  // dice el bueno de Javier que no lo pone el subject, pero bueno que poder se
  // puede hacer
...
bool ServerResponse::_saveUploadedFile(UploadedFile const &file,
...
void ServerResponse::_handleFileUpload(ClientRequest const &clientRequest,
                                       const Config &config) {
  ...
  if (!location || !location->getIsUpload()) {
    _response = buildErrorResponse(
        500, "Upload not configured for this location", config);
    return;
  }
  ...
  if (uploadDir.empty()) {
    _response =
        buildErrorResponse(500, "Upload directory not configured", config);
    return;
  }
  // verificar si tiene permisos de escritura
  if (access(uploadDir.c_str(), W_OK) != 0) {
    _response =
        buildErrorResponse(500, "Upload directory not writable", config);
    return;
  }
  for (size_t i = 0; i < files.size(); i++) {
    ...
    if (!_validateUploadedFile(file, config)) {
      _response =
          buildErrorResponse(400, "Invalid file: " + file._filename, config);
      return;
    }

    if (!_saveUploadedFile(file, uploadDir)) {
      _response = buildErrorResponse(
          500, "Failed to save file: " + file._filename, config);
      return;
    }
  }
