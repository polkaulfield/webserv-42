...
char** Cgi::_populateEnv(ClientRequest& clientRequest, Config& config, std::string petition, std::string htmlPath)
{
	...
	envVars.push_back("HTTP_USER_AGENT=" + extractUserAgent(petition));
	envVars.push_back("HTTP_ACCEPT=" + extractAccept(petition));
	envVars.push_back("HTTP_ACCEPT_LANGUAGE=" + extractAcceptLanguage(petition));
	envVars.push_back("HTTP_CONNECTION=" + extractConnection(petition));
...
std::string	Cgi::execScript(std::string htmlPath, std::string petition,
						ClientRequest& clientRequest, Config& config) {

    std::cout << "Executing script!!" << std::endl;
	std::string	interpreter = determineInterpreter(htmlPath);

	char** env = _populateEnv(clientRequest, config, petition, htmlPath);
	std::string postData = clientRequest.getData();
