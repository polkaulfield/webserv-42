...
// This function is to be called before executing the cgi behind fork. We set
// all the parameters inside the params map as a new environment for the child
// cgi process
...
char **Cgi::_populateEnv(const ClientRequest &clientRequest,
                         const Config &config) {
  // TODO: We need to pass all the needed env variables from info to cgi after
  // forking
  ...
  envVars.push_back("HTTP_USER_AGENT=" +
                    clientRequest.getHeaderValue("User-Agent"));
  envVars.push_back("HTTP_ACCEPT=" + clientRequest.getHeaderValue("Accept"));
  envVars.push_back("HTTP_ACCEPT_LANGUAGE=" +
                    clientRequest.getHeaderValue("Accept-Language"));
...
std::string Cgi::getCGIOutput(char **args, char **env,
...
std::string Cgi::execScript(const ClientRequest &clientRequest,
                            const Config &config) {
  ...
  if (!interpreter.empty()) {
    char *args[] = {
        (char *)interpreter.c_str(), // el (char *) entiendo que es para
                                     // castearlo a char * sin const
